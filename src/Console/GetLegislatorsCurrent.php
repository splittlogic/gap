...
class GetLegislatorsCurrent extends Command
{
    protected $signature = 'splittlogic:GetLegislatorsCurrent';

    protected $description = 'Download YAML file of Current Legislators';

    public function handle()
    {

      // Start script logging
      \Artisan::call('splittlogic:ScriptLogging --name=GetLegislatorsCurrent');
      ...
      $file = file_get_contents('https://theunitedstates.io/congress-legislators/legislators-current.yaml');
      ...
      foreach ($files as $f)
      {
        ...
        if (Str::contains($f,'legislators-current'))
        {
          $clfiles[] = $f;
        }
      ...
      if (empty($clfiles))
      {
        ...
        file_put_contents(public_path('files') . '/' . now()->format('Y-m-d') . '_legislators-current.yaml', $file);
        $yaml = $file;
        ...
        if ($file == $file2)
        {
          ...
          file_put_contents(public_path('files') . '/' . now()->format('Y-m-d') . '_legislators-current.yaml', $file2);
          $yaml = $file2;
          $this->info('New file saved');
